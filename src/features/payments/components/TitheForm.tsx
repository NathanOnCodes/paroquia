...
const stripePromise = env.LOCAL_DEMO_MODE
  ? Promise.resolve(null)
  : loadStripe(env.NEXT_PUBLIC_CHAVE_PUBLICA_STRIPE);
...
export default function TitheForm({ communities, recurringOnly = false }: TitheFormProps) {
  const router = useRouter();
  const [form, setForm] = useState<FormState>({ ...initialState, is_recurring: recurringOnly });
    ...
    try {
      const payload = {
        ...
        amount_cents: amountCents,
         is_recurring: recurringOnly || form.is_recurring,
  ...
  async function handlePay() {
    ...
    const returnUrl = `${window.location.origin}/doacoes/sucesso`;
    ...
    <Box sx={{ maxWidth: 520, mx: "auto" }}>
      {stage === "form" ? (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {env.LOCAL_DEMO_MODE && (
            <Alert severity="info">
              Modo de demonstração local: pagamentos reais estão desativados.
            </Alert>
          )}
          {error && (
          ...
          {!recurringOnly && (
            <FormControlLabel
              ...
              label="Quero que a contribuição seja recorrente todo mês"
            />
          )}
          <FormControlLabel
            control={
